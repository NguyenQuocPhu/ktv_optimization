...
from ktv_routing.contract import GeoPoint
...
def _geometry_centroid(geometry: dict[str, Any]) -> GeoPoint:
    ...
    if total_area > 0:
        return GeoPoint(
            lat=weighted_latitude / total_area,
            lng=weighted_longitude / total_area,
        )
    if fallback_points:
        return GeoPoint(
            lat=sum(point[1] for point in fallback_points)
            / len(fallback_points),
            lng=sum(point[0] for point in fallback_points)
            / len(fallback_points),
        )
